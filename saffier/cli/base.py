...
def revision(
    ...
    null_fields: list[str] | tuple[str, ...] | None = None,
) -> None:
    """Create a new migration revision file.

    When `autogenerate` is enabled, Alembic compares the current database state
    with the registered Saffier models to populate the revision script. The
    optional ``null_fields`` selectors are active only for the duration of this
    command so model metadata can temporarily render selected fields as
    nullable, allowing generated migrations to add required columns to tables
    that already contain data.

    Args:
        app: Active Saffier application or migration wrapper owner.
        directory: Migration script directory.
        message: Revision message stored in the generated file.
        autogenerate: Whether Alembic should compare database and model
            metadata.
        sql: Whether Alembic should emit SQL instead of executing commands.
        head: Target head revision for the new migration.
        splice: Whether a non-head revision can be used as the base.
        branch_label: Optional branch label for the generated revision.
        version_path: Optional Alembic version path.
        revision_id: Optional explicit revision identifier.
        null_fields: Optional ``Model:field`` or ``:field`` selectors that
            should be rendered as nullable while Alembic reads model metadata.
    """
    with with_force_fields_nullable(null_fields):
        ...
        signal_payload = {
            ...
            "null_fields": tuple(null_fields or ()),
        }
...
def migrate(
    ...
    This is the higher-level alias behind the `makemigrations` command. The
    command always runs Alembic autogeneration and shares the same temporary
    forced-nullable field scope as ``revision`` so both public commands produce
    equivalent migration scripts.
    ...
    Args:
        ...
        message: Revision message stored in the generated file.
        sql: Whether Alembic should emit SQL instead of executing commands.
        head: Target head revision for the generated migration.
        splice: Whether a non-head revision can be used as the base.
        branch_label: Optional branch label for the generated revision.
        version_path: Optional Alembic version path.
        revision_id: Optional explicit revision identifier.
        arg: Extra Alembic ``-x`` arguments for custom env.py scripts.
        null_fields: Optional ``Model:field`` or ``:field`` selectors that
            should be rendered as nullable while Alembic reads model metadata.
    """
    with with_force_fields_nullable(null_fields):
