"""Concrete model implementations exposed by Saffier."""
...
class Model(ModelRow, DeclarativeMixin, AdminMixin):
    """Primary ORM model class used by application code.

    `Model` adds the persistence lifecycle on top of `SaffierBaseModel`. It
    knows how to validate writes, execute field hooks, emit signals, reload
    server-generated values, and serialize itself through the same declared
    field map that powers query construction.
    """
...
class ReflectModel(Model, SaffierBaseReflectModel):
    """Concrete model class used for database-reflected tables and views.

    Reflected models keep the same persistence and serialization API as normal
    models, but their SQLAlchemy table definition is loaded from an existing
    database structure rather than generated from field declarations.
    """
