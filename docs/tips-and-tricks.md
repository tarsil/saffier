...
The examples are more focused on the [Ravyn](https://ravyn.dymmond.com) as the author is the
...
This is probably the easiest way to place the connection details and particulary for Ravyn since
...
your code. In the case of Ravyn:
...
[Ravyn directives](https://ravyn.dymmond.com/management/directives/)

### The settings

As mentioned before we will have a settings file with database connection properties assembled.

```python title="my_project/configs/settings.py"
{!> ../docs_src/tips/settings.py !}
```
