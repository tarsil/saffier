...
Saffier also exposes lifecycle signals for migration commands:

```python
from saffier.core.signals import post_migrate, pre_migrate
```

Use `connect_via()` to listen for a specific migration command. Supported sender names are
`revision`, `upgrade`, and `downgrade`. The `migrate` command creates an autogenerated revision, so
it emits the `revision` sender with `autogenerate=True`.

```python
{!> ../docs_src/signals/migrations.py !}
```

Migration receivers can be synchronous or asynchronous. They receive the Alembic `config`, command
arguments such as `revision` or `message`, and `_async_wrapper`, which can be used by synchronous
receivers that need to run async application work.
