...
class QueryManager(ModelManager):
    ...
...
class AvailableManager(ModelManager):
    async def all(self, **kwargs):
        return await super().all(is_available=True)
...
class Product(saffier.Model):
    ...
    fields = {
        ...
        "is_available": saffier.BooleanField(default=True),
    }
...
async def test_model_active_crud():
    users = await User.active.all()
    assert users == []

    for _ in range(0, 4):
        await User.query.create(name="Test", is_active=False)
