...
from saffier.core.connection.database import DatabaseURL
...
def test_database_url_options_and_replace():
    ...
    assert str(new) == "edgedb://localhost/test_mydatabase?pool_size=20&ssl=true"

    new = url.replace(port=5656)
    assert new.port == 5656
    assert str(new) == "edgedb://localhost:5656/mydatabase?pool_size=20&ssl=true"
