...
def test_migrate_applies_forced_nullable_scope_during_revision(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Verify autogenerated migrations run inside the forced-nullable context.

    The public command option is parsed outside ``saffier.cli.base``, but the
    base command is responsible for wrapping Alembic autogeneration. This test
    patches Alembic's revision function and inspects the template-facing list
    literal while the command is running, then confirms the context is reset
    ...
    def fake_revision(*args, **kwargs):
        """Capture active forced-nullable selectors at Alembic call time.

        Args:
            *args: Positional Alembic command arguments.
            **kwargs: Keyword Alembic command arguments.
        """
        rendered_context.append(force_fields_nullable_as_list_string())

    monkeypatch.setattr(cli_base.command, "revision", fake_revision)

    cli_base.migrate(
        app=app,
        directory="migrations",
        message="nullable",
        null_fields=["User:name", ":slug"],
    )

    assert rendered_context == ['[("", "slug"), ("User", "name")]']
