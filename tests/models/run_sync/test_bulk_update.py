...
async def test_bulk_update():
    ...
    products = saffier.run_sync(Product.query.all())
...
async def test_bulk_update_with_relation():
    album = saffier.run_sync(Album.query.create(name="foo"))
    album2 = saffier.run_sync(Album.query.create(name="fighters"))
    ...
    tracks = saffier.run_sync(Track.query.all())
    for track in tracks:
        track.album = album2

    saffier.run_sync(Track.query.bulk_update(tracks, fields=["album"]))
    tracks = saffier.run_sync(Track.query.all())
