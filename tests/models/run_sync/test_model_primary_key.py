...
async def test_model_default_primary_key():
    ...
    users = saffier.run_sync(User.query.filter())
...
async def test_model_custom_primary_key():
    profile = saffier.run_sync(Profile.query.create(name="Test", language="EN", age=18))
    profiles = saffier.run_sync(Profile.query.filter())
