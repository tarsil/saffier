from typing import ClassVar

import pytest

import saffier
from saffier import Manager
from saffier.core.db.querysets.base import QuerySet
from saffier.testclient import DatabaseTestClient as Database
from tests.settings import DATABASE_URL

database = Database(url=DATABASE_URL)
models = saffier.Registry(database=database)

pytestmark = pytest.mark.anyio


class UserManager(Manager):
    def get_queryset(self) -> QuerySet:
        queryset = super().get_queryset().filter(is_admin=False)
        return queryset


class User(saffier.Model):
    password = saffier.CharField(max_length=128)
    username = saffier.CharField(max_length=150, unique=True)
    email = saffier.EmailField(max_length=120, unique=True)
    is_active = saffier.BooleanField(default=True)
    is_student = saffier.BooleanField(default=True)
    is_teacher = saffier.BooleanField(default=True)
    is_admin = saffier.BooleanField(default=True)
    is_staff = saffier.BooleanField(default=False)
    created_at = saffier.DateField(auto_now_add=True)

    mang: ClassVar[Manager] = UserManager()

    class Meta:
        registry = models
        tablename = "manager_simple_users"


@pytest.fixture(autouse=True, scope="function")
async def create_test_database():
    await models.create_all()
    yield
    await models.drop_all()


@pytest.fixture(autouse=True)
async def rollback_connections():
    with database.force_rollback():
        async with database:
            yield


async def test_can_create_record():
    await User.mang.create(password="12345", username="user", email="test@test.com")
    assert User.mang.instance is None

    users = await User.mang.all()

    assert len(users) == 0

    user = await User.mang.create(
        password="12345", username="user2", email="test1@test.com", is_admin=False
    )
    assert user.mang.instance is user
    User.mang._fooobar = True
    assert hasattr(User.mang, "_fooobar")
    assert not hasattr(user.mang, "_fooobar")

    users = await User.mang.all()

    assert len(users) == 1
    assert users[0].pk == user.pk
    assert hasattr(users[0].mang, "_fooobar")
